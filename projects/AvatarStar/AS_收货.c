...
#define fRESULT_TIMEOUT 1
#define fRESULT_FAILED  2
#define fRESULT_DISABLE 4
uint fcheck(char* name){
    ...
    uint c2 = pts[i].c2;
    if(getPx(x, y)==c1){
        SendMessage(hc, WM_MOUSEMOVE  , MK_LBUTTON, MAKELPARAM(x, y));
        SendMessage(hc, WM_LBUTTONDOWN, MK_LBUTTON, MAKELONG  (x, y));
        SendMessage(hc, WM_LBUTTONUP  , MK_LBUTTON, MAKELONG  (x, y));
    }
...
uint fbtn(char* name){
    ...
    while(1){
        uint c;
        c = getPx(x, y);
        if(c==c1||c==c2||c==c3 && clicked==0){
            SendMessage(hc, WM_MOUSEMOVE  , MK_LBUTTON, MAKELPARAM(x, y));
            SendMessage(hc, WM_LBUTTONDOWN, MK_LBUTTON, MAKELPARAM(x, y));
            clicked++;
        }
        if(perfTick() - t >= fTIMEOUT) return fRESULT_TIMEOUT;
        if(c==c3){
            SendMessage(hc, WM_LBUTTONUP  , MK_LBUTTON, MAKELPARAM(x, y));
            return fRESULT_CLICKED;
        }
    }
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    *stdin = *_fdopen(_open_osfhandle((long)GetStdHandle(STD_INPUT_HANDLE), _O_TEXT), "r" );
    ...
    clr(0x07);
    printf("\x9 AvatarStar_机械化收货工具 @雪星(Snowstar.Here@gmail.com)\n");
    printf("\n");
    WinLoad();
    clr(0x0F);
    uint index=0;
