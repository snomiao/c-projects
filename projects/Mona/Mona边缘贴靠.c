...
uint WinLoad(){
    ...
    SetWindowPos(hwndGame, 0, 0, 0, 1920, 1080, SWP_NOOWNERZORDER);
    // SetWindowsPos(hwndGame, 0,
    //     //  0 + cw - dw,
    //     0,
    //     // 20 + cy - dy,
    //     0,
    ...
    /* 图像区 */
    pnt.x = pnt.y = 0;
    ClientToScreen(hwndGame, &pnt);
    GetClientRect(hwndGame, &rct);
    dx = pnt.x;                 dy = pnt.y;
